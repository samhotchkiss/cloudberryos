...
- No Tailscale binding for the admin panel (V1.1+).
...
| Admin panel | M4 (version 0.2.0). Python stdlib `http.server`, root service, `127.0.0.1:8766`, **token auth mandatory from the first commit** (spec below). In M1–M3 setup records only an `admin_panel` on/off flag, **default `off`** — this deliberately supersedes the older draft's "local only by default" (the panel doesn't exist until M4; a parent enables it explicitly) | Loopback is reachable by the child by design; binding is not auth |
...
--admin-panel local|off                (default off; records flag only until M4)
...
- The admin panel requires token auth even on loopback, because the child can reach 127.0.0.1 by design.
...
- **Admin panel V1.1+**: Tailscale binding, thumbnails/embed status, update checks, non-root privilege split (dedicated service user + root oneshot for apply), Linux-account auth.
